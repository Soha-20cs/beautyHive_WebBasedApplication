...
$date = date("Y-m-d");
$sql = "SELECT * FROM appointments WHERE appointment_date > NOW() AND doc_id = '$doc_id' ORDER BY appointment_time ASC";
...
if (mysqli_num_rows($result) > 0) {
  ...
  <form method='POST' action=''>
    <select name='action' onchange='this.form.submit()'>
      <option value='processing'".($row["remark"] == 'processing' ? " selected" : "").">Processing</option>
      <option value='approved'".($row["remark"] == 'approved' ? " selected" : "").">Approved</option>
      <option value='cancelled'".($row["remark"] == 'cancelled' ? " selected" : "").">Cancelled</option>
    </select>
    ...
    echo "No appointments scheduled for Future.";
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  // Update the appointment record with the new action value
  $sql = "UPDATE appointments SET remark = '$action' WHERE id = '$id'";
  if (mysqli_query($conn, $sql)) {
    echo "<script>alert('Appointment updated successfully.');window.location.href = 'upcoming_appointments.php';</script>";
  } else {
